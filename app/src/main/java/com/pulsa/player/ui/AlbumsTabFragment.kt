...
import com.pulsa.player.ui.adapter.AlbumGridAdapter
...
class AlbumsTabFragment : Fragment() {
    ...
    private var adapter: AlbumGridAdapter? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val a = AlbumGridAdapter { album -> openAlbum(album) }
        adapter = a
        view.findViewById<RecyclerView>(R.id.list).apply {
            val density = resources.displayMetrics.density
            val widthPx = resources.displayMetrics.widthPixels
            val span = (widthPx / density / 118f).toInt().coerceIn(2, 5)
    ...
    private fun openAlbum(album: Album) {
        (activity as? MainActivity)?.openAlbum(album)
    }
    ...
    fun title(): String = requireContext().getString(R.string.tab_albums)
}
